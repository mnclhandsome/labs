...
class Dashboard extends CI_Controller {
	...
	public function index(){
		...
		if($this->session->userdata('userinfo')){
			$pharmacyUserdetails = $this->session->userdata('userinfo');
			if($pharmacyUserdetails["is_pharmacy"] == 1){
				$pharmacyUserdetails = $this->session->userdata('userinfo');

				$data['getPharmacyDetails'] = $this->pharmacy_model->getUserPharmacyDetails($pharmacyUserdetails['user_id']);
				...
				$this->load->view('pharmacy/includes/sidebar', $data);
				$this->load->view('pharmacy/view_dashboard', $data);
				$this->load->view('pharmacy/includes/footer');

			}else{
				$data['msg'] = 'You don not have an access.';
	...
	public function addMedicine(){
		...
		if($this->session->userdata('userinfo')){
			...
			if($pharmacyUserdetails["is_pharmacy"] == 1){
				$pharmacyUserdetails = $this->session->userdata('userinfo');
				$data['getPharmacyDetails'] = $this->pharmacy_model->getUserPharmacyDetails($pharmacyUserdetails['user_id']);
				$medicine_list = $this->medicine_model->get_all_medicine_lists($pharmacyUserdetails['user_id']);
			...
			$this->load->view('pharmacy/includes/header', $data);
			$this->load->view('pharmacy/includes/sidebar', $data);
			$this->load->view('pharmacy/view_add_medicine', $data);
			$this->load->view('pharmacy/includes/footer');
			}
		}
	}
